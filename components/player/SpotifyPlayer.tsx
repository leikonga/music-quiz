...
export function SpotifyPlayer({ accessToken }: SpotifyPlayerProps) {
  const [player, setPlayer] = useState<
    (Spotify.Player & SpotifyOutOfBetaPolyfill) | null
  >(null);
  ...
  const { setCurrentTrack, setRevealed, startTime, spotifyTracks } =
    usePlayer();
      ...
      playerObject.addListener("player_state_changed", (state) => {
        ...
        if (state) {
          ...
          if (
            ...
            playerStateRef.current?.track_window.current_track.id !=
